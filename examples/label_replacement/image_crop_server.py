...
Takes an image and bounding box coordinates, crops the image(s) in memory,
and returns the crop payloads for downstream classification.
...
def crop():
	...
	try:
		...
		original_image_b64 = base64.b64encode(image_bytes).decode("ascii")
		...
		# Crop images in memory for downstream classification.
		...
		for idx, box_info in enumerate(boxes_data):
			try:
				...
				# Keep encoded crop bytes for downstream services.
				buffer = BytesIO()
				cropped.save(buffer, format="JPEG", quality=95)
				encoded_crop = base64.b64encode(buffer.getvalue()).decode("ascii")

				# Keep only successful crops for downstream classification.
				...
				logger.info("Prepared in-memory crop %d (size: %s)", idx, cropped.size)
		...
		return (
			jsonify(
				{
					"status": "success",
					"count": len(classification_inputs),
